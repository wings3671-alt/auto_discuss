...
def write(path, text):
    ...
    取舍要讲清楚:os.replace 的改名是原子的(读者只会看到完整旧或完整新);
    原地写放弃了这个原子性,换取 inode 稳定。用 r+ 而非 w 已尽量收窄风险——
    w 一打开就把文件清空、有一段「文件为空」的空窗;r+ 不清空,旧内容一直
    保留到新内容写完、再 truncate 掉多余旧尾巴。但写入动作本身仍非原子:
    并发读者理论上可能读到写一半的中间态(亚毫秒级窗口)。对本程序可接受
    ——单进程顺序写、轮询间隔 5 秒、编辑器会自动重读纠正;中途崩溃留半截
    文件的风险由每轮调用前的时间戳备份兜底(见 make_backup /
    backup_state_file)。文件不存在时退回 w 模式新建。"""
